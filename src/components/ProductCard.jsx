...
const ProductCard = ({ product }) => {
  ...
  const onAddToCart = (product) => dispatch(addToCart(product));
  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden group">
      <div className="relative overflow-hidden bg-gray-100 h-56">
        <img src={product.thumbnail} alt={product.title} className="w-full h-full object-contain scale-90 group-hover:scale-100 transition-transform duration-300" />
        ...
        <div className="absolute top-3 left-3 bg-purple-600 text-white px-2 py-1 rounded text-xs font-semibold uppercase">{product.category}</div>
      </div>

      <div className="p-5">
        <h3 className="text-lg font-bold text-gray-800 mb-2 line-clamp-2 group-hover:text-purple-600 transition-colors">{product.title}</h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-200">
          <div>
            ...
            {product.discountPercentage > 0 && <div className="text-xs text-gray-500 line-through">${(product.price / (1 - product.discountPercentage / 100)).toFixed(2)}</div>}
          </div>
          <button
